...
class SpidyBrain:
    ...
    def _generate_with_retry(self, prompt_text, image=None):
        """
        Attempts to generate content with fallback logic:
        1. Gemini Flash (Fast)
        2. Gemini Pro (Backup)
        3. Local Brain (Offline/Unlimited)
        """
        # 1. Try Gemini Flash
        if "flash" in self.gemini_models:
            try:
                if image:
                    response = self.gemini_models["flash"].generate_content([prompt_text, image])
                else:
                    response = self.gemini_models["flash"].generate_content(prompt_text)
                return response.text
            except Exception as e:
                if "429" in str(e) or "ResourceExhausted" in str(e):
                    sys.stderr.write(f"[WARN] Gemini Flash Rate Limit (429). Switching to Pro...\\n")
                else:
                    sys.stderr.write(f"[WARN] Gemini Flash Error: {e}\\n")

        # 2. Try Gemini Pro
        if "pro" in self.gemini_models:
            try:
                if image:
                     response = self.gemini_models["pro"].generate_content([prompt_text, image])
                else:
                    response = self.gemini_models["pro"].generate_content(prompt_text)
                return response.text
            except Exception as e:
                sys.stderr.write(f"[WARN] Gemini Pro Error: {e}\\n")
    ...
    def decide_intent(self, user_query, image_data=None, persona="cyberpunk"):
        ...
        if not decision:
            try:
                response_text = self._generate_with_retry(prompt_text, image=processed_image)
